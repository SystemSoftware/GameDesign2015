   ...
   public void Rotate(float acceleration, float up, float right)
    {
       ...
       Vector3 accelerationVector = (-ship.forward * (acceleration * 50)) - ship.forward * 40 + (-ship.up * 10 * Mathf.Sin(acceleration/2)*up*1.5f) + (ship.right * 10 * Mathf.Sin(acceleration/2)*right*2);
